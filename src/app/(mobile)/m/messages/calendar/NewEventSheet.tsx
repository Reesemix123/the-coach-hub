              ...
              {EVENT_TYPES.map(t => (
                <button key={t.value} type="button" onClick={() => setEventType(t.value)}
                  className={`rounded-lg px-2.5 py-1.5 text-xs font-semibold transition-colors ${
                    eventType === t.value ? 'bg-[var(--bg-pill-active-accent)] text-[var(--accent-text)]' : 'bg-[var(--bg-card-alt)] text-[var(--text-secondary)] active:bg-[var(--bg-pill-inactive)]'
                  }`}>
                  {t.label}
                </button>
              ))}
                  ...
                  return (
                    <button key={ch.value} type="button" onClick={() => !disabled && setChannel(ch.value)} disabled={disabled}
                      className={`flex-1 rounded-lg px-3 py-2 text-xs font-semibold transition-colors ${
                        channel === ch.value ? 'bg-[var(--bg-pill-active-accent)] text-[var(--accent-text)]'
