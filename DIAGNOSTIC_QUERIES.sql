-- ============================================
-- DIAGNOSTIC QUERIES FOR GAME/VIDEO ID MISMATCH
-- ============================================
...
-- 4. Check if game_id in file_path matches game_id column
SELECT
  v.id,
  v.name,
  v.game_id as db_game_id,
  SPLIT_PART(v.file_path, '/', 1) as filepath_game_id,
  CASE
    WHEN v.game_id = SPLIT_PART(v.file_path, '/', 1) THEN '✅ Match'
    ELSE '❌ MISMATCH'
  END as status,
  v.file_path
FROM videos v
