-- Migration 135: Add game_ids array parameter to calculate_team_metrics
-- ============================================================================
-- PURPOSE:
--   Support cumulative filtering (e.g., "Through Week 5") by allowing
--   an array of game IDs to be passed instead of just a single game ID.
--   Preserves the output structure from migration 132.
-- ============================================================================

-- Drop the existing function first (it has a different signature)
...
CREATE OR REPLACE FUNCTION calculate_team_metrics(
  ...
  p_game_ids UUID[] DEFAULT NULL  -- NEW: Array of game IDs for cumulative filtering
) RETURNS JSONB AS $$
DECLARE
  v_result JSONB;
...
-- Grant execute permission to both old and new function signatures
