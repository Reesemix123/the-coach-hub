-- Migration 130: Fix All Player Stats Functions (Complete Rewrite)
-- ============================================================================
-- PURPOSE:
--   Complete fix for all 8 player stats functions that were returning 500 errors.
--
-- ROOT CAUSES IDENTIFIED:
--   1. Migration 129 removed the games table join, breaking team filtering
--   2. RB function used 'ball_carrier' but migration 123 defines 'rusher'
--   3. Completion detection relied on pi.is_complete which isn't always set
--
-- FIXES IN THIS MIGRATION:
--   1. Restore video_filter CTE with games.team_id join
--   2. Use 'rusher' for RB stats (matching migration 123)
--   3. Use pp.result = 'success' for pass completions (seed script pattern)
--   4. Add SECURITY DEFINER for consistent RLS behavior
--   5. Simplify column references to only those that exist
-- ============================================================================
...
BEGIN
  WITH video_filter AS (
    -- Filter videos by team via games table join
  ...
  qb_aggregated AS (
    SELECT
      ...
      COUNT(*) FILTER (WHERE pp_result = 'success') AS completions,
      COALESCE(SUM(yards_gained) FILTER (WHERE pp_result = 'success'), 0) AS passing_yards,
...
BEGIN
  ...
  rb_rushing AS (
    ...
    WHERE pp.team_id = p_team_id
      AND pp.phase = 'offense'
      -- FIX: Use 'rusher' from migration 123, not 'ball_carrier'
  ...
  rb_aggregated AS (
    SELECT
      ...
      COALESCE(MAX(r.yards_gained), 0) AS longest_run,
    ...
    LEFT JOIN (
      SELECT
        player_id,
        COUNT(*) AS targets,
        COUNT(*) FILTER (WHERE pp_result = 'success' OR is_touchdown = TRUE) AS receptions,
        COALESCE(SUM(yards_gained) FILTER (WHERE pp_result = 'success' OR is_touchdown = TRUE), 0) AS rec_yards,
...
BEGIN
  ...
  receiver_aggregated AS (
    SELECT
      ...
      COALESCE(SUM(yards_gained) FILTER (WHERE pp_result = 'success' OR is_touchdown = TRUE), 0) AS rec_yards,
      COUNT(*) FILTER (WHERE is_touchdown = TRUE) AS rec_tds,
      COUNT(*) FILTER (WHERE yards_gained >= 15 AND (pp_result = 'success' OR is_touchdown = TRUE)) AS explosive_catches
...
BEGIN
  ...
  kicker_aggregated AS (
    SELECT
      ...
      jersey_number,
      primary_position,
      -- Field goals: use kick_result = 'made' (from migration 039)
      ...
      -- For XPs, check pp_result = 'success' since kick_result doesn't have xp_ values
      COUNT(*) FILTER (WHERE participation_type = 'kicker' AND pp_result = 'success') AS total_makes,
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'totalPoints', (fg_made * 3)
    )
    ORDER BY (fg_made * 3) DESC
  )
  INTO v_result
...
BEGIN
  ...
  returner_aggregated AS (
    SELECT
      ...
      jersey_number,
      primary_position,
      -- Note: special_teams_unit uses 'kick_return' not 'kickoff_return' per migration 039
      COUNT(*) FILTER (WHERE special_teams_unit = 'kick_return') AS kick_returns,
      COALESCE(SUM(yards_gained) FILTER (WHERE special_teams_unit = 'kick_return'), 0) AS kick_return_yards,
      COUNT(*) FILTER (WHERE special_teams_unit = 'kick_return' AND is_touchdown = TRUE) AS kick_return_tds,
      COALESCE(MAX(yards_gained) FILTER (WHERE special_teams_unit = 'kick_return'), 0) AS longest_kick_return,
      ...
      COALESCE(MAX(yards_gained) FILTER (WHERE special_teams_unit = 'punt_return'), 0) AS longest_punt_return
...
-- ============================================================================
-- Migration 130 fixes all 8 player stats functions:
--
-- Key fixes:
-- 1. Restored video_filter CTE with games.team_id join (was removed in 129)
-- 2. Fixed get_rb_stats to use 'rusher' instead of 'ball_carrier'
-- 3. Use pp.result = 'success' for pass completion detection
-- 4. Added SECURITY DEFINER for consistent RLS behavior
-- 5. Fixed get_returner_stats to use 'kick_return' (not 'kickoff_return')
-- 6. Simplified kicker stats (removed invalid xp_made/xp_missed values)
--
-- All functions now:
-- - Filter by team via games table JOIN
-- - Use correct participation_type values from migration 123
-- - Use pp.result = 'success' pattern from seed script
-- - Have SECURITY DEFINER for reliable execution
-- ============================================================================
