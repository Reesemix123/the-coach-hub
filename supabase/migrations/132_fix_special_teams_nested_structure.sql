-- Migration 132: Fix calculate_team_metrics to include nested special teams structure
-- ============================================================================
-- PURPOSE:
--   Migration 125 overwrote migration 122's changes and removed the nested
--   structure for kickoff, punt, returns, fieldGoal, pat, and fgBlock.
--   The SpecialTeamsReport.tsx expects these nested objects.
--
-- ISSUE:
--   SpecialTeamsReport accesses: metrics.specialTeams?.kickoff?.kickoffs
--   But migration 125 only has: metrics.specialTeams.puntReturns (flat)
--
-- FIX:
--   1. Add missing variable declarations (v_kickoffs, v_punts, etc.)
--   2. Update QUERY 4 to include kickoff and punt counts
--   3. Add nested structure to specialTeams output
-- ============================================================================
...
BEGIN
  ...
  -- Now includes: kickoffs, touchbacks, punts, punt_yards, longest return
  -- ============================================================================
  SELECT
  ...
  v_result := jsonb_build_object(
    ...
    'specialTeams', jsonb_build_object(
      ...
      'fieldGoal', jsonb_build_object(
        ...
        'blocked', 0  -- Would need to track our FGs blocked by opponent
      ),
      ...
      'fgBlock', jsonb_build_object(
        'blocks', v_fg_blocked,
        'blocksRecovered', 0,  -- Would need additional tracking
        'blocksReturnedForTD', 0  -- Would need additional tracking
      )
    ),
...
COMMENT ON FUNCTION calculate_team_metrics IS 'Calculate all comprehensive football metrics for a team. Fixed in migration 132 to include nested special teams structure (kickoff, punt, returns, fieldGoal, pat, fgBlock) required by SpecialTeamsReport.tsx.';
