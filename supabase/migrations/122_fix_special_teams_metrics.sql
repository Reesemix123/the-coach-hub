-- Migration 122: Fix calculate_team_metrics function for special teams
-- The function was somehow modified and no longer properly queries special teams data.
-- This migration resets the special teams queries to use the correct boolean flags.
...
BEGIN
  ...
  -- ============================================================================
  -- QUERY 4: Special Teams Metrics (FG, PAT, Returns)
  -- IMPORTANT: No is_opponent_play filter - special teams includes both
  -- ============================================================================
  SELECT
    ...
    COUNT(*) FILTER (WHERE is_punt = TRUE),
    COALESCE(SUM(kick_distance) FILTER (WHERE is_punt = TRUE), 0),
    COUNT(*) FILTER (WHERE is_kickoff = TRUE),
    COUNT(*) FILTER (WHERE is_touchback = TRUE AND is_kickoff = TRUE)
  INTO
  ...
  -- Includes both flat structure (for backward compatibility) and nested structure
  -- ============================================================================
  v_result := jsonb_build_object(
    ...
    'specialTeams', jsonb_build_object(
      ...
      -- Nested structure for SpecialTeamsReport
      ...
      'returns', jsonb_build_object(
        ...
        'longestReturn', 0  -- Would need additional tracking
      ),
      ...
      'fgBlock', jsonb_build_object(
        'blocks', 0,  -- Would need additional tracking
        'blocksRecovered', 0,
...
COMMENT ON FUNCTION calculate_team_metrics IS 'Calculate all comprehensive football metrics for a team. Fixed in migration 122 to properly query special teams data using boolean flags (is_field_goal_made, is_punt_return, etc.)';
