-- ============================================================================
-- Migration 034: Automatic Drive Stats Recalculation
-- ============================================================================
...
BEGIN
  ...
  SELECT
    ...
    bool_or(result_type IN ('touchdown', 'pass_touchdown')),
    bool_or(result_type = 'field_goal')
  INTO
    v_has_turnover,
    v_has_touchdown,
    v_has_field_goal
  ...
  IF v_has_turnover THEN
    v_result := 'turnover';
    v_points := 0;
  ELSIF v_has_touchdown THEN
    v_result := 'touchdown';
    v_points := 6;  -- Base touchdown points (excludes PAT)
  ELSIF v_has_field_goal THEN
    v_result := 'field_goal';
    v_points := 3;
  ELSIF v_last_play_result = 'punt' THEN
    v_result := 'punt';
    v_points := 0;
  ELSIF v_last_play_result = 'turnover_on_downs' THEN
    v_result := 'downs';
    v_points := 0;
  ELSIF v_last_play_result = 'safety' THEN
    v_result := 'safety';
    v_points := 2;
  ELSE
    v_result := 'end_half';
