-- Migration 125: Fix Red Zone Efficiency Calculation
-- Purpose: Use drives table for accurate red zone efficiency instead of play-level yard_line
-- Problem: Season Overview shows 0% red zone efficiency while Drive Analysis shows 100%
-- Root cause: calculate_team_metrics was counting plays with yard_line >= 80, not drives that reached red zone

-- ============================================================================
-- Update the calculate_team_metrics function to use drives table for red zone
-- ============================================================================
...
BEGIN
  ...
  -- Red zone stats removed - will use drives table instead
  -- ============================================================================
  SELECT
    -- Games
    COUNT(DISTINCT g.id),
  ...
  -- ============================================================================
  -- QUERY 1b: Offensive Red Zone Stats (from drives table)
  -- This is the correct way to calculate red zone efficiency
  -- ============================================================================
  SELECT
    COUNT(*) FILTER (WHERE reached_red_zone = TRUE),
    COUNT(*) FILTER (WHERE reached_red_zone = TRUE AND result = 'touchdown')
  INTO
    v_red_zone_attempts,
    v_red_zone_touchdowns
  FROM drives d
  ...
  -- ============================================================================
  -- QUERY 2b: Defensive Red Zone Stats (from drives table)
  -- Opponent drives that reached our red zone
  -- ============================================================================
  SELECT
    COUNT(*) FILTER (WHERE reached_red_zone = TRUE),
    COUNT(*) FILTER (WHERE reached_red_zone = TRUE AND result = 'touchdown')
...
-- Update comment
COMMENT ON FUNCTION calculate_team_metrics IS 'Calculate all 28 comprehensive football metrics for a team. Uses drives table for accurate red zone efficiency. Supports filtering by game, date range, and opponent. Returns structured JSONB with offense, defense, special teams, and overall metrics.';
